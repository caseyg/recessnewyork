<div class="hero gallery js-flickity col-xs-12" data-flickity-options='{"autoPlay": true, "wrapAround": true, "pageDots": false}'>
	...
	foreach( $slides as $slide): ?>
		...
		<a class="gallery-cell col-xs-12" href="<?php echo $hero->url() ?>">	
			<img src="<?php if($hero->images()->find('hero.png')): ?><?php echo $hero->images()->find('hero.jpg', 'hero.png')->first()->crop('977','400')->url() ?><?php else: ?><?php echo $hero->images()->first()->crop('977','400')->url() ?><?php endif ?>" width="977" height="400" alt="<?php if($hero->subtitle()): ?><?php echo $hero->subtitle() ?><?php endif ?><?php if($hero->title()): ?>: <?php echo $hero->title() ?><?php endif ?>">
